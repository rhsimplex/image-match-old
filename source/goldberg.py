...
class ImageSignature(object):
    ...
    def __init__(self, n=9, crop_percentiles=(5,95), P=None, diagonal_neighbors=True, identical_tolerance=2/255., n_levels=2):
        ...
        assert identical_tolerance >= 0. and identical_tolerance <= 1.,\
    ...
    def compute_differentials(grey_level_matrix,  diagonal_neighbors=True):
        ...
        if diagonal_neighbors:
            ...
            upper_left_neighbors = sum([np.diagflat(np.insert(\
                    np.diff(np.diag(grey_level_matrix, i))\
                    , 0, 0), i) for i in diagonals])
